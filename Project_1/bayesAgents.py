...
def constructBayesNet(gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            # Add the edge from each house to the observation variable
            edges.append((FOOD_HOUSE_VAR, obsVar))
            edges.append((GHOST_HOUSE_VAR, obsVar))

            # Set up observation variable domain
            variableDomainsDict[obsVar] = OBS_VALS

    # Add edges from the position variables to the house variables
    ...
    # Set up position and house variables domains
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        jointFactor = inference.inferenceByVariableElimination(
            self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, outcomeEvidence in probsAndOutcomes:
            leftValue, rightValue = self.computeEnterValues(outcomeEvidence, enterEliminationOrder)
            bestValue = max(leftValue, rightValue)
            expectedValue += prob * bestValue
