...
class BayesNet(object):
    ...
    def sameGraph(self, other):
        ...
        # normalize edge dicts, so both key+values are compared
        def _normalize_edges(d):
            return {k: tuple(sorted(v)) for k, v in d.items()}

        sameInEdges = _normalize_edges(self.__inEdges) == _normalize_edges(other.__inEdges)
        sameOutEdges = _normalize_edges(self.__outEdges) == _normalize_edges(other.__outEdges)
